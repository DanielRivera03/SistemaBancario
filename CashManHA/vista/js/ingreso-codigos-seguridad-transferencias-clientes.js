...
//$("#generar-nuevos-codigos-clientes").submit(function(event) {
    $(document).on('click', '#registro-nuevos-codigos-transferencias-clientes', function(event){
        ...
        $.ajax({
            type: "POST",
            url: "../controlador/cGestionesCashman.php?cashmanhagestion=generador-codigos-transferencias-clientes",
            data: parametros,
            // MENSAJE DE ESPERA -> PREVIO A CONFIRMACION / RECHAZO DE PETICION
            beforeSend: function(objeto) {
                $("#resultados_ajax").html('<div class="alert alert-light solid alert-dismissible fade show"><img style="width: 30px;" class="img-fluid" src="../vista/images/Hourglass.gif"><strong>¡Generando C&oacute;digo de Seguridad!</strong> Espere un momento por favor...</div>');
            },
            success: function(datos) {
                ...
                // SI LA PETICION ES ACEPTADA, MOSTRAR MENSAJE DE CONFIRMACION DE PETICION EXITOSA -> MENSAJE FLOTANTE
                    ...
                    setTimeout(function () {
                        AlertaUsuarioToastr_RegistroExitoso("Código de seguridad generado exitosamente", "Registro Exitoso");
                        $('#registro-nuevos-codigos-transferencias-clientes').prop('disabled', true);
