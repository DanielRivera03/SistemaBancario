$(document).ready(function(){  
    ...
    $(document).on('click', '#cerrar-cuentas-ahorro', function(e){
     var IDCuentasClientesGestion = $(this).data('id'); // ID ENVIADA POR GET DESDE EL CONTROLADOR HACIA EL MODELO
     CerrarCuentas(IDCuentasClientesGestion); // ID UNICO DE USUARIOS
     e.preventDefault();
    });
   });
   function CerrarCuentas(IDCuentasClientesGestion){ 
    swal.fire({
        title: 'Cerrar Cuenta de Ahorros?',
        text: "Al hacer clic en el botón confirmar, esta cuenta de ahorros será cerrada y dejará de estar operativa en nuestro sistema",
        type: 'warning',
     ...
     preConfirm: function() {
       return new Promise(function() {
          $.ajax({
          url: '../controlador/cGestionesCashman.php?cashmanhagestion=envio-datos-cerrar-cuentas-ahorros-clientes&idcuentas='+IDCuentasClientesGestion,
          type: 'POST',
          ...
          .done(function(respuesta){ // SI MODELO EJECUTA PETICION, REALIZA PETICION
            if(respuesta=="OK"){
                AlertaUsuarioMostrar("Cuenta Cerrada","Su petición se ha completado con éxito","success");
